...
impl Token {
    ...
    pub fn owner_of(&self, id: U256) -> AccountHash {
        let key = format!("owner_{}", id);
        self.query_contract(&key).unwrap_or_default()
    }
    ...
    pub fn approve(&mut self, spender: AccountHash, id: U256, sender: Sender) {
        self.call(
            sender,
            "approve",
            runtime_args! {
    ...
    pub fn mint(&mut self, recipient: AccountHash, sender: Sender, id: U256) {
        self.call(
            sender,
            "mint",
            runtime_args! {
    ...
    pub fn transfer(&mut self, recipient: AccountHash, id: U256, sender: Sender) {
        self.call(
            sender,
            "transfer",
            runtime_args! {
